...
class Bellmanford:
    ...
    def find_shortest_path(self):
        ...
        for vertex_iteration in range(len(self.vertex_list) - 1):
            for each_edge in self.edge_list:
                if each_edge.start_node.min_distance + each_edge.weight < each_edge.end_node.min_distance:
                    each_edge.end_node.previous_node = each_edge.start_node
                    each_edge.end_node.min_distance = each_edge.start_node.min_distance + each_edge.weight

        for edge in self.edge_list:
            if self.check_cycle(edge) is True:
                print('Negative Cycle Detected')
                return

    def check_cycle(self, edge):
        if edge.start_node.min_distance + edge.weight < edge.end_node.min_distance:
            self.cycle = True
            return True
        else:
            return False
